$(function(){
    ...
    //slider
    $('.content__bxslider').bxSlider();

    // accordions panels

    var $main = $('.pannels__main');
    var $text = $('.pannels__text');
    $main.addClass('.pannels__main-active');
    $($main).children('.panells__title').addClass('pannels__title--active');

    $main.siblings().hide();


    $main.on('click', function(){
        $($main).removeClass('.pannels__main-active');
        $('.panells__title').removeClass('.pannels__main-active');

        $(this).siblings($text).slideToggle();
        $(this).toggleClass('.pannels__main-active');
        $(this).children('.panells__title').toggleClass('pannels__title--active');
    ...
    var remove = [];
    var newSkills = [];
    skills = _.map(data, 'skills');
    skills = _.flatten(skills);
    skills = _.uniq(skills);
    remove = _.remove(skills, function(s){
        return s == skills[0];
    });
    newSkills = _.concat(skills, remove);
    newSkills = _.sortBy(newSkills);
    console.log('First task:', newSkills); // В самом массиве data.skills = что б было по алфавиту Lorem я изменил на lorem !!!

    var name = [];
    name = _.sortBy(data, 'friends');
    name = _.map(name, 'name');
    name = _.sortBy(name, 'friends');
    name = _.reverse(name);
    console.log('Second task:', name);

    var friends = [];
    friends = _.map(data, 'friends');
    friends = _.flatten(friends);
    friends = _.map(friends, 'name');
    friends = _.uniq(friends);
    console.log('Third task:', friends);
